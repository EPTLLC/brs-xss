...
def test_confidence_branches_reflection_and_payload():
    ...
    refl_min = SimpleNamespace(reflection_type=None, completeness=0.0, characters_preserved=0.0)
    c1 = cc.calculate_confidence(refl_min, {"context_type": "html_content"}, "<s>")
    assert c1 > c0

    # Reflection with known type and bonuses
    refl = SimpleNamespace(reflection_type=SimpleNamespace(value="partial"), completeness=1.0, characters_preserved=1.0)
    ctx = {"context_type": "html_attribute", "tag_name": "a", "attribute_name": "href", "filters_detected": ["html"], "encoding_detected": "html"}
