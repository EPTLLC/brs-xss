...
def _mk_reflection(exact=True):
    class RP:
        def __init__(self):
            self.reflection_type = type("T", (), {"value": "exact" if exact else "filtered"})()
    ...
    class RR:
        def __init__(self):
            ...
            self.overall_reflection_type = type("T", (), {"value": "exact" if exact else "filtered"})()
...
def test_scoring_engine_stats_and_weights():
    ...
    se.update_weights(ScoringWeights(impact=0.5, exploitability=0.4, context=0.1, reflection=0.0))
    ...
    res = se.score_vulnerability("<script>alert(1)</script>", reflection, ctx, MagicMock(status_code=200))
