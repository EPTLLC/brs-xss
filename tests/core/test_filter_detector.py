...
def test_detect_encoding_patterns_and_none():
    ...
    assert fd.detect_encoding("\\u003Cdiv\\u003E") == EncodingType.UNICODE_ESCAPING.value
...
def test_analyze_filter_strength_levels_and_recommendations():
    ...
    recs = fd.get_filter_recommendations([FilterType.HTML_ENTITY_ENCODING.value, FilterType.URL_ENCODING.value])
    assert any("entities" in r.lower() for r in recs) and any("url" in r.lower() for r in recs)
