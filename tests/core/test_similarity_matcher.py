...
def test_encoded_reflections_and_encoders():
    ...
    assert "url_encoding" in kinds and "html_entities" in kinds and "hex_escape" in kinds
...
def test_calculate_reflection_quality_metrics():
    ...
    assert q1["similarity"] == 1.0 and q1["completeness"] == 1.0 and q1["accuracy"] == 1.0
    q2 = m.calculate_reflection_quality("abcdef", "abc")
    assert 0.0 < q2["similarity"] < 1.0 and 0.0 < q2["completeness"] <= 1.0 and 0.0 < q2["char_preservation"] <= 1.0
