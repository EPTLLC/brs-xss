...
def test_specific_technique_application(monkeypatch):
    gen = PayloadGenerator(config=_cfg(max_payloads=5))
    monkeypatch.setattr(gen.context_generator, "get_context_payloads", lambda c, i: ["<script>alert(1)</script>"])
    r = gen.generate_single_payload({"context_type": "html_content"}, technique=EvasionTechnique.URL_ENCODING)
...
def test_bulk_generation_quota_and_error(monkeypatch):
    ...
    assert all(p.description in ("Context-matrix", "Comprehensive") for p in res["javascript"])
...
def test_bulk_generation_catches_waf_exception(monkeypatch):
    ...
    monkeypatch.setattr(gen.waf_evasions, "generate_waf_specific_payloads", lambda base, wafs: (_ for _ in ()).throw(RuntimeError("waf boom")))
...
def test_evasion_exception_is_logged(monkeypatch):
    ...
    gen = PayloadGenerator(config=cfg)
    monkeypatch.setattr(gen.context_generator, "get_context_payloads", lambda c, i: ["<svg onload=alert(1)>"])
    # Break one technique
    monkeypatch.setattr(gen.evasion_techniques, "apply_case_variations", lambda s: (_ for _ in ()).throw(RuntimeError("evasion boom")))
    res = gen.generate_payloads({"context_type": "html_content"}, detected_wafs=[], max_payloads=5)
...
def test_high_threshold_filters_out_all(monkeypatch):
    ...
    gen = PayloadGenerator(config=cfg)
    monkeypatch.setattr(gen.context_generator, "get_context_payloads", lambda c, i: ["A"]) 
    res = gen.generate_payloads({"context_type": "html_content"}, detected_wafs=[], max_payloads=5)
    assert res == []
...
def test_blind_context_mapping(monkeypatch):
    ...
    for ctx_in, ctx_out in [
        ...
        res = gen.generate_payloads({"context_type": ctx_in}, detected_wafs=[], max_payloads=5)
