...
def test_html_context_detector_core_branches():
    ...
    assert d.detect_html_context(html, pos_comment, "marker") == ContextType.HTML_COMMENT
    # attribute branch
    ...
    assert info["tag_name"] in ("a", "div") and isinstance(info["dangerous_attributes"], list)
...
def test_js_context_detector_core_branches():
    ...
    assert d.detect_js_context(html, pos, "MARK") in (ContextType.JS_STRING, ContextType.JAVASCRIPT, ContextType.JS_OBJECT)
    ...
    assert isinstance(details, dict) and (details["is_in_function_call"] or details["is_in_variable_assignment"] or details["is_in_event_handler"] or details["is_in_conditional"] is False)
