...
def test_basic_headers_analyzer_xss_protection():
    ...
    assert r_missing.security_level == SecurityLevel.WEAK and r_missing.value == "MISSING"
    r_off = a.analyze_xss_protection("0")
    assert r_off.security_level == SecurityLevel.VULNERABLE and any("disabled" in v.lower() for v in r_off.vulnerabilities)
...
def test_basic_headers_analyzer_frame_and_content_type_and_referrer_and_hsts():
    ...
    assert r_frame_missing.security_level == SecurityLevel.WEAK and r_frame_missing.value == "MISSING"
    ...
    assert r_xcto_missing.security_level == SecurityLevel.WEAK and r_xcto_missing.value == "MISSING"
    ...
    assert r_hsts_missing_dirs and r_hsts_missing_dirs.security_level == SecurityLevel.MODERATE
