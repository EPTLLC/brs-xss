...
from brsxss.utils.validators import URLValidator, ParameterValidator, ConfigValidator, FileValidator
...
def test_config_validator_ranges_and_user_agent():
    ...
    assert not r.valid and any("between" in e for e in r.errors) and any("User agent" in w for w in r.warnings)
...
def test_file_validator_extension_and_overwrite(tmp_path):
    ...
    assert r.valid and any("Unusual" in w for w in r.warnings) and any("already exists" in w for w in r.warnings)
