...
async def test_system_e2e_scan_and_report(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scanner.payload_generator, "generate_payloads", lambda ctx, wafs, max_payloads=None: [P()])

    vulns = await scanner.scan_url("http://test.local/search", method="GET", parameters={"q": "probe"})
    ...
    for i, v in enumerate(vulns, 1):
        ...
        if hasattr(sev, "value"):
            sev = sev.value
        items.append(VulnerabilityData(
            id=f"xss_{i}",
