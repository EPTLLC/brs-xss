...
def _node(nt: NodeType, value: str, ln: int, col: int, **kw) -> ASTNode:
    return ASTNode(node_type=nt, value=value, children=[], line_number=ln, column=col, **kw)
...
def test_dom_analyzer_classification_and_recommendations(monkeypatch):
    ...
    src = _node(NodeType.FUNCTION_CALL, "location.hash", 1, 1, function_name="location.hash")
    sink = _node(NodeType.PROPERTY_ACCESS, "document.body.innerHTML", 2, 5, object_name="document.body", property_name="innerHTML")
    mapping = SourceSinkMapping(source_node=src, sink_node=sink, data_path=[src, sink], vulnerability_confidence=0.9, risk_factors=["hash", "innerHTML"])

    # Patch JS parser inside analyzer
    monkeypatch.setattr(analyzer, "js_parser", type("P", (), {
    ...
    monkeypatch.setattr(analyzer.vulnerability_classifier, "classify_vulnerability", lambda s, k, d: (VulnerabilityType.DIRECT_ASSIGNMENT, RiskLevel.HIGH))

    vulns = analyzer.analyze_javascript("document.body.innerHTML = location.hash;")
    assert vulns and vulns[0].vulnerability_type == VulnerabilityType.DIRECT_ASSIGNMENT and vulns[0].risk_level == RiskLevel.HIGH
