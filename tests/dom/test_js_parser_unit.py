...
def test_js_parser_tracks_assignments_and_stats(monkeypatch):
    ...
    assign_node = ASTNode(node_type=NodeType.VARIABLE_ASSIGNMENT, value="x = y", children=[], line_number=1, column=1, variable_name="x")
    call_node = ASTNode(node_type=NodeType.FUNCTION_CALL, value="eval(x)", children=[], line_number=2, column=1, function_name="eval")
