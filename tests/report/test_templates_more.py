...
def test_sarif_template_minimal(tmp_path: Path):
    ...
    files = gen.generate_report([_vuln("low")], ScanStatistics(scan_duration=0.5), {"url": "https://x"})
...
def test_junit_template_empty(tmp_path: Path):
    ...
    gen = ReportGenerator(cfg)
    files = gen.generate_report([], ScanStatistics(scan_duration=0.0), {"url": "https://x"})
