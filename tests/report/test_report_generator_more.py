...
def _vuln(id_: str, title: str, desc: str, sev: str, url: str = "https://t") -> VulnerabilityData:
...
def test_prepare_report_data_kb_merge_preserve_fields(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(rg, "get_vulnerability_details", lambda c: {
        "title": "KB Title",
...
def test_generate_single_format_sarif_calls_reporter_and_writes_file(tmp_path, monkeypatch):
    ...
    assert all(k in saved.get("scan_info_keys", []) for k in ["command_line", "duration", "end_time", "machine", "start_time", "targets_scanned"])
...
def test_get_top_vulnerable_urls_and_generator_stats(tmp_path):
    ...
    assert "html" in stats["supported_formats"] and "json" in stats["configured_formats"]
