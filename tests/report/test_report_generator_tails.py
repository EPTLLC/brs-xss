...
def test_generate_report_with_unsupported_format_logs_error(tmp_path: Path):
    cfg = ReportConfig(output_dir=str(tmp_path), formats=[ReportFormat.HTML, ReportFormat.CSV])
    gen = ReportGenerator(cfg)
    files = gen.generate_report([_v("1", "low", "reflected_xss")], ScanStatistics(scan_duration=0.1), {"url": "https://x"})
...
def test_generate_summary_report_file(tmp_path: Path):
    gen = ReportGenerator(ReportConfig(output_dir=str(tmp_path), formats=[ReportFormat.HTML]))
    scan_results = [
        {
            "vulnerabilities": [
                {
